...
export const POST: APIRoute = async ({ params, request }) => {
  try {
    ...
    // Check if feature request exists and is published
    const { data: featureRequest, error: fetchError } = await supabaseAdmin
      .from('feature_requests')
      .select('id, votes_count, status, hidden')
      .eq('id', id)
      .single();
    ...
    const { data: updated, error: updateError } = await supabaseAdmin
      .from('feature_requests')
      .update({ votes_count: featureRequest.votes_count + 1 })
      .eq('id', id)
      .select('votes_count')
      .single();
    ...
    if (updateError) {
      ...
      // Don't fail the request if vote was inserted
    }
